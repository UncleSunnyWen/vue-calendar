...
const calendar = {
    ...
    solar2lunar: function (y, m, d) { //参数区间1900.1.31~2100.12.31
        ...
        if (month == 1 && day == 1) {
            ...
            if (isLeap ? day == this.leapDays(year) : this.monthDays(month - 1) == 29 ? day == 30 : day == 29) festival.push('除夕');
        }
        //公历节日
        if (m == 1 && d == 1) {
            ...
            if (d == this.getSunday(y, m, 2)) festival.push('母亲节');
        } else if (m == 6) {
            if (d == 1) festival.push('儿童节');
            if (d == 4) festival.push('青年节');
            if (d == this.getSunday(y, m, 3)) festival.push('父亲节');
        } else if (m == 7 && d == 1) {
            festival.push('建党节');
        } else if (m == 8 && d == 1) {
            festival.push('建军节');
    ...
    getSolar(date) {
        ...
        var cM = parseInt(D[1]);
        var cD = parseInt(D[2]);
        return this.lunar2solar(cY, cM, cD, isLeapMonth);
    },
};
